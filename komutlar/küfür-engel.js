...
exports.run = async (client, message, args) => {
     ...
     let kapalırow = new ActionRowBuilder()
          .addComponents(
               new ButtonBuilder()
                    .setCustomId(`küfür-engel-aktif`)
                    .setLabel("Sistemi Aktif Et")
                    .setEmoji("🤬")
                    .setStyle(ButtonStyle.Success)
          )
     if (db.get(`${message.guild.id}_küfürengel`) === true) {
          row = aktifrow;
     } else {
          row = kapalırow;
     }

     let sistem = db.get(`${message.guild.id}_küfürengel`) ? "🟢 **Aktif**" : "🔴 **Kapalı**"

     let embed = await message.reply({
          embeds: [Embed(message.author.username, message.author.avatarURL(), "Küfür Engel Sistemi", `Aşağıda küfür engel sistemiyle ilgili bilgiler mevcuttur;`, [
     ...
     collector.on('collect', async (i) => {
          ...
          if (i.customId === "küfür-engel-kapat") {
               await db.delete(`${i.guild.id}_küfürengel`)
               if (db.get(`${i.guild.id}_küfürengel`) === true) {
                    row = aktifrow;
               } else {
                    row = kapalırow;
               }
               let sistem2 = await db.get(`${message.guild.id}_küfürengel`) ? "🟢 **Aktif**" : "🔴 **Kapalı**"
               await i.reply({ content: `✅ | Başarıyla küfür engel sistemini kapattım.`, ephemeral: true })
               await embed.edit({
                    embeds: [Embed(message.author.username, message.author.avatarURL(), "Link Engel Sistemi", `Aşağıda küfür engel sistemiyle ilgili bilgiler mevcuttur;`, [
                         { name: `Sistem`, value: `${sistem2}` },
                    ], `İsteyen ${message.author.username}`, message.author.avatarURL(), null)], components: [row]
               })
          }
          if (i.customId === "küfür-engel-aktif") {
               await db.set(`${i.guild.id}_küfürengel`, true)
               if (db.get(`${i.guild.id}_küfürengel`) === true) {
                    row = aktifrow;
               } else {
                    row = kapalırow;
               ...
               let sistem2 = await db.get(`${message.guild.id}_küfürengel`) ? "🟢 **Aktif**" : "🔴 **Kapalı**"
               await i.reply({ content: `✅ | Başarıyla küfür engel sistemini aktif ettim.`, ephemeral: true })
               await embed.edit({
                    embeds: [Embed(message.author.username, message.author.avatarURL(), "Küfür Engel Sistemi", `Aşağıda küfür engel sistemiyle ilgili bilgiler mevcuttur;`, [
                         { name: `Sistem`, value: `${sistem2}` },
                    ], `İsteyen ${message.author.username}`, message.author.avatarURL(), null)], components: [row]
...
exports.help = {
     name: "küfür-engel",
     desc: 'Küfür engelleme sistemini açar/kapatırsınız.'
}